...
def _news_raw_extra_json(item: NewsItem) -> Optional[str]:
    if not item.raw_extra:
        return None
    try:
        return json.dumps(item.raw_extra, ensure_ascii=False)
    except (TypeError, ValueError):
        return None


def _parse_news_raw_extra(val: Optional[str]) -> Optional[Dict[str, Any]]:
    if not val:
        return None
    try:
        out = json.loads(val)
...
class SQLiteStorageMixin:
    ...
    def _init_tables(self, conn: sqlite3.Connection, db_type: str = "news") -> None:
        ...
        if db_type == "news":
            ...
            self._migrate_news_items_raw_extra(conn)

        conn.commit()

    def _migrate_news_items_raw_extra(self, conn: sqlite3.Connection) -> None:
        """Ensure news_items.raw_extra exists (older DBs)."""
        cur = conn.execute("PRAGMA table_info(news_items)")
        cols = {row[1] for row in cur.fetchall()}
        if "raw_extra" not in cols:
            conn.execute("ALTER TABLE news_items ADD COLUMN raw_extra TEXT")
                            ...
                            if existing:
                                ...
                                cursor.execute("""
                                    UPDATE news_items SET
                                        ...
                                        rank = ?,
                                        mobile_url = ?,
                                        raw_extra = COALESCE(?, raw_extra),
                                        last_crawl_time = ?,
                                        crawl_count = crawl_count + 1,
                                        updated_at = ?
                                    WHERE id = ?
                                """, (item.title, item.rank, item.mobile_url,
                                      _news_raw_extra_json(item),
                                ...
                                cursor.execute("""
                                    ...
                                    (title, platform_id, rank, url, mobile_url,
                                     ...
                                     raw_extra, created_at, updated_at)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)
        ...
        try:
            ...
            for row in rows:
                ...
                items[platform_id].append(NewsItem(
                    ...
                    raw_extra=_parse_news_raw_extra(row[10]) if len(row) > 10 else None,
                    news_item_db_id=row[0],
                ))
